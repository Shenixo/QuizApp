    ...
    <ul className="flex gap-6 flex-col">
      {options.map(({ icon, option, color, boxColor }, idx) => (
        <li
          ...
          `}
        >
          <div className={`w-16 h-16 flex items-center justify-center rounded-[.8rem] ${boxColor}`}>
            <FontAwesomeIcon icon={icon} className={`text-3xl ${color}`} style={{ strokeWidth: "0.2px" }} />
          </div>
          {option}
        </li>
      ))}

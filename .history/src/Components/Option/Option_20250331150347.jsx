    ...
    <ul className="flex gap-6 flex-col">
      {options.map(({ icon, option, color, boxColor }, idx) => (
        <li
          ...
          `}
        >
          <div className={`w-16 h-16 flex items-center justify-center rounded-[1rem] ${boxColor}`}>
            <FontAwesomeIcon icon={icon} className={`text-3xl ${color}`} style={{ strokeWidth: "0.3px" }} />
          </div>
          {option}
        </li>
      ))}
